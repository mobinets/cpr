function [conflict_flag, conflict_pos, pkg2_pre_bin, conflict_arg] = find_conflict_multi(G_processing, lora_set, d_downchirp_cfo_array, d_upchirp_cfo_array, snr_rate_array)
    dine = lora_set.dine;
    ...
    conflict_fft_merge = zeros(0);      % 保存冲突发现所需要的FFT
    for GW_count = 1:GW_num
        ...
        conflict_fft_merge_tmp = normalize(conflict_fft_merge_tmp, 2, 'range');
        if GW_count == 1
            conflict_fft_merge = conflict_fft_merge_tmp .* snr_rate_array(GW_count);
        else
            conflict_fft_merge = conflict_fft_merge + conflict_fft_merge_tmp .* snr_rate_array(GW_count);
        end
    end
    ...
    % 把所有FFT窗口主峰旁边的峰都置0
    for col = 1:size(Peak_pos,1)
        for row = 1:size(Peak_pos,2)
            condition_1 = abs(Peak_pos(col,row)-[1:fft_x]) < fft_x*leakage_width1;   % 找到fft_x*leakage_width1范围内的旁瓣
            condition_2 = abs(Peak_pos(col,row)-[1:fft_x]) > fft_x*leakage_width2;
            condition_3 = Peak_pos(col,row) ~= [1:fft_x];                  % 避免把主峰置0
            sidelobe_index = (condition_1 | condition_2) & condition_3;
            conflict_fft_merge(col,sidelobe_index) = 0;
        end
    end
    ...
    conflict_alter = zeros(9,length(conflict_maybe));   % 第一行flag，第二行冲突位置，第三行重复出现的bin，第四行最高峰的值和最大残差的比值，第五行第六行系数k，第七行第八行残差，第九行重复出现次数
    for i = 1:length(conflict_maybe)
        ...
        if con_1 && (con_2 || con_3) && con_4
            ...
            if conflict_pos >= 3 && conflict_pos <= Pkg_length - 3
                ...
                conflict_alter(4,i) = condition_3(max_pos);       % 第四行：最高峰的值和平均残差
                conflict_alter(5,i) = p_1(1);
                conflict_alter(6,i) = p_2(1);
                conflict_alter(7,i) = residual_1;
                conflict_alter(8,i) = residual_2;
                conflict_alter(9,i) = superimpose_count;
    ...
    elseif size(alter_index,2) == 1
        ...
        conflict_arg = conflict_alter(5:10,alter_index)';
    else
        ...
        conflict_arg = alter_index_t(5:10,max_pos)';    
    end