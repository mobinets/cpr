function [Pkg1_bin, Pkg2_bin] = get_bin(lora_set, Pkg1_samples_fft_merge, Pkg2_samples_fft_merge, Peak_rate_full, conflict_pos, pkg2_pre_bin)
    ...
    if Peak_rate_full >= 1 % 包1峰比包2峰高
        ...
        % 矫正Pkg1_map_Pkg2的值（前后2个bin）
        if pkg2_pre_bin < fft_x*0.4
            Pkg1_map_Pkg2 = circshift(Pkg1_map_Pkg2, -1);
        end
        for windows_tmp = conflict_pos:Pkg_length
            min_pos = max([1, Pkg1_map_Pkg2(windows_tmp)-2]);
            max_pos = min([fft_x, Pkg1_map_Pkg2(windows_tmp)+2]);
            [tmp, fix_tmp] = max( Pkg2_samples_fft_merge(windows_tmp-conflict_pos+1, min_pos : max_pos ));
            Pkg1_map_Pkg2(windows_tmp) = Pkg1_map_Pkg2(windows_tmp) + fix_tmp - 3;
        end
        ...
        % 矫正Pkg1_map_Pkg2的值（前后2个bin）
        if pkg2_pre_bin < fft_x*0.4
            Pkg2_map_Pkg1 = circshift(Pkg2_map_Pkg1, 1);
        end
        for windows_tmp = 1:Pkg_length - conflict_pos + 1
            min_pos = max([1, Pkg2_map_Pkg1(windows_tmp)-2]);
            max_pos = min([fft_x, Pkg2_map_Pkg1(windows_tmp)+2]);
            [tmp, fix_tmp] = max( Pkg1_samples_fft_merge(conflict_pos+windows_tmp-1,  min_pos:max_pos ));
            Pkg2_map_Pkg1(windows_tmp) = Pkg2_map_Pkg1(windows_tmp) + fix_tmp - 3;
        end
