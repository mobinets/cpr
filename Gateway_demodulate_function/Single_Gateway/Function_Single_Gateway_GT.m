...
function [Pkg1_ture, Pkg2_ture, G_processing, conflict_pos, pkg2_pre_bin, conflict_arg, Peak_rate_full] = Function_Single_Gateway_GT(G0, lora_set, Verification_path, cfo, windows_offset)
    ...
    if conflict_flag == 3   % 如果发现了冲突的话
        % 计算峰值比
        [Peak_rate_full] = get_peak_rate(G0, lora_set, d_downchirp_cfo, conflict_pos, pkg2_pre_bin);
    end
