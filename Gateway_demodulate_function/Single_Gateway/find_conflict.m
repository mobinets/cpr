function [conflict_flag, conflict_pos, pkg2_pre_bin, conflict_arg] = find_conflict(G0, lora_set, d_downchirp_cfo, times)
    d_sf = lora_set.sf;
    d_bw = lora_set.bw;
    dine = lora_set.dine;
    ...
    for col = 1:size(Peak_pos,1)
        for row = 1:size(Peak_pos,2)
            ...
            % x = (abs(Peak_pos(col,row)-[1:fft_x]) < fft_x*leakage_width1 | abs(Peak_pos(col,row)-[1:fft_x]) > fft_x*leakage_width2) & Peak_pos(col,row) ~= [1:fft_x];
            % conflict_fft_merge(col, (abs(Peak_pos(col,row)-conflict_fft_merge(col,:)) < fft_x*leakage_width1 | abs(Peak_pos(col,row)-conflict_fft_merge(col,:)) < fft_x*leakage_width2) & Peak_pos(col,row) ~= conflict_fft_merge(col,:)) = 0;
        end
    end
