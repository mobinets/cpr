function [cfo, winoffset] = nscale_get_cfo(G0, lora_set)
    d_sf = lora_set.sf;
    d_bw = lora_set.bw;
    dine = lora_set.dine;
...
%     upchirp_samples = G0((Preamble_length-1)*dine+1 : Preamble_length*dine);
%     cor = abs(xcorr(upchirp_samples, d_upchirp, dine*1));
%     [~, cor_up] = max(cor(dine+1 : dine*2));
    cor_up = 1;

    upchirp_samples = G0((Preamble_length-1)*dine+1 : (Preamble_length+4)*dine);
    cor = abs(xcorr(upchirp_samples, d_downchirp, 5*dine));
    [~, cor_down] = max(cor(dine*5+1 : dine*10));

    if mod(cor_down, dine) < dine * 0.5
        der = mod(cor_down-cor_up, dine)/2;
        cfo = -der*d_bw/dine;
        winoffset = cor_up + der;
    else
        der = mod(cor_up-cor_down, dine)/2;
        cfo = der*d_bw/dine;
        winoffset = cor_up + der;
    end
