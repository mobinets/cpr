function [pkg_bin] = nscale_get_bin_pkg1_tmp(G0, lora_set, cfo, algin_windows, pkg2_algin_windows)
    d_bw = lora_set.bw;
    dine = lora_set.dine;
    ...
    pkg2_preamble_bin = (1 - decimal_pkg2) * fft_x;   % 计算包2在对齐包1窗口时的bin
    ...
    if conflict_pos < Pkg_length  % 去掉包2的preamble的峰
        min_pos = max([1, fix(pkg2_preamble_bin-fft_x*0.05)]);
        max_pos = max([1, fix(pkg2_preamble_bin+fft_x*0.05)]);
        samples_fft_tmp = samples_fft(dectect_windows, :);
        samples_fft_tmp(1 : min_pos) = 0;  samples_fft_tmp(max_pos : fft_x) = 0;
        [~, max_bin] = max(samples_fft_tmp);  % 获得修正后的包2bin值
        condition_1 = abs(max_bin-[1:fft_x]) < fft_x*0.05;   % 找到fft_x*leakage_width1范围内的所有包1的峰
        condition_2 = abs(max_bin-[1:fft_x]) > fft_x*0.95;
        sidelobe_index = condition_1 | condition_2;
        if conflict_pos <= Preamble_length + 4
            samples_fft(1:conflict_pos+3-Preamble_length, sidelobe_index) = 0;
        else
            samples_fft(conflict_pos-Preamble_length-4 : conflict_pos+3-Preamble_length, sidelobe_index) = 0;
        end
    end
    ...
    for windows = 1 : size(samples_fft, 1)
    ...
    if conflict_pos >= Preamble_length + 6
        if conflict_pos <= Pkg_length - 7
            pkg_bin(1:conflict_pos - Preamble_length - 5 + 8) = Peak1_pos(1:conflict_pos - Preamble_length - 5 + 8, 1);
        else
            pkg_bin(1:end) = Peak1_pos(1:length(pkg_bin), 1);
        end
    end