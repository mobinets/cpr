function [Pkg1_ture, Pkg2_ture, conflict_pos] = Nscale(G0, lora_set, Verification_path)
    ...
    Pkg2_ture = 0;
    % cor pkg1
    [algin_windows, detect_flag, pkg1_cor, cor_floor] = correlate(G0, lora_set);
    
    % cor pkg2
    [pkg2_algin_windows, conflict_flag] = nscale_correlate_pkg2(G0, lora_set, pkg1_cor, cor_floor, algin_windows);
    conflict_pos = ceil(pkg2_algin_windows/lora_set.dine);
    % disp(pkg2_algin_windows/lora_set.dine);
    ...
    % see_zeropaddingfft(G_pkg1, lora_set);
...
%     [cfo, windows_offset] = get_cfo_winoff(G_pkg1, lora_set, d_downchirp, d_upchirp);
    % disp(cfo);
    [cfo, windows_offset] = nscale_get_cfo(G_pkg1, lora_set);
    ...
    [pkg_bin] = nscale_get_bin_pkg1_tmp(G_pkg1, lora_set, cfo, algin_windows, pkg2_algin_windows);
...
%     [cfo, windows_offset] = get_pkg2_cfo_winoff(G_pkg2, lora_set, d_downchirp, d_upchirp);
    [cfo, windows_offset] = nscale_get_cfo(G_pkg2, lora_set);
    ...
    [pkg_bin] = nscale_get_bin_tmp(G_pkg2, lora_set, cfo, algin_windows, pkg2_algin_windows);
