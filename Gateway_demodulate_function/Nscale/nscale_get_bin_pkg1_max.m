function [pkg_bin] = nscale_get_bin_pkg1_max(G0, lora_set, cfo, algin_windows, pkg2_algin_windows)
    d_bw = lora_set.bw;
    dine = lora_set.dine;
    ...
    [~, max_pos] = max(nscale_factor, [], 2);
    pkg_bin = zeros(1,length(max_pos));
    for windows = 1:length(max_pos)
        pkg_bin(windows) = Peak1_pos(windows, max_pos(windows));
    end

    if conflict_pos >= Preamble_length + 6
        pkg_bin(1:conflict_pos - Preamble_length - 5) = Peak1_pos(1:conflict_pos - Preamble_length - 5, 1);
    end